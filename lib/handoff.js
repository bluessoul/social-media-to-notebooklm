...
function sourceTypeForUrl(url) {
  ...
  if (value.includes('doc88.com')) return 'doc88';
  return 'unknown';
}
...
function buildBilibiliMarkdown({ url, bvid, p, source, language, asrModel, title, items }) {
...
function buildBilibiliHandoff({ url, bvid, p, source, title, outputDir, files }) {
  const normalizedFiles = compactFiles({
...
function buildDoc88Handoff({ url, title, pCode, pageCount, files }) {
  const normalizedFiles = compactFiles({ pdf: existingFile(files.pdf) });
  return {
    version: 1,
    source_type: 'doc88',
    source_url: url,
    title: title || pCode || 'Doc88 document',
    document_id: pCode || null,
    page_count: pageCount || null,
    files: normalizedFiles,
    suggested_upload: normalizedFiles.pdf ? 'pdf' : null
  };
}

function buildEmlHandoff({ sourcePath, title, emailCount, attachments, files }) {
  const normalizedFiles = compactFiles({ markdown: existingFile(files.markdown) });
  return {
    version: 1,
    source_type: 'eml',
    source_file: existingPath(sourcePath),
    title: title || 'EML email archive',
    email_count: emailCount || 0,
    attachment_names: Array.isArray(attachments) ? attachments : [],
    files: normalizedFiles,
    suggested_upload: normalizedFiles.markdown ? 'markdown' : null
  };
}
