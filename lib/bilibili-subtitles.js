...
async function browserSubtitle(url, rate) {
  ...
  if (!browser) {
    const error = new Error('CDP_UNAVAILABLE: 找不到已开启 CDP 的 Chrome/Edge。该结果不代表视频没有字幕；在 Codex 中应转用已连接的 Chrome 浏览器，检查播放器中的 [data-lan="ai-zh"] 中文 AI 选项。');
  ...
  const result = await page.evaluate(async (playbackRate) => {
    ...
    if (!click('[data-lan="ai-zh"]')) throw new Error('AI_SUBTITLE_NOT_FOUND: 当前播放器没有暴露中文 AI 字幕选项。不要将官方字幕接口为空误报为该错误。');
