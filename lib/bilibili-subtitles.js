...
if (!inputUrl || args.help) {
  console.log('用法: run.bat --url "https://www.bilibili.com/video/BV..." [--output <目录>] [--playback-rate 4]');
...
(async () => {
  ...
  fs.writeFileSync(path.join(outDir, `${stem}.srt`), toSrt(result.items), 'utf8');
  fs.writeFileSync(path.join(outDir, `${stem}.json`), JSON.stringify({ url: inputUrl, bvid, source: result.source, title: result.title, items: result.items }, null, 2), 'utf8');
  console.log(`字幕已导出: ${path.join(outDir, `${stem}.srt`)}`);
