...
function doc88WorkDir(outputDir, pCode) {
  const key = String(pCode || 'document').replace(/[^A-Za-z0-9_-]/g, '_');
  return path.join(outputDir, `.doc88-work-${key}`);
}

function existingNonEmpty(filePath) {
  return fs.existsSync(filePath) && fs.statSync(filePath).size > 0;
}

function writeDoc88Progress(workDir, progress) {
  fs.writeFileSync(path.join(workDir, 'progress.json'), `${JSON.stringify(progress, null, 2)}\n`, 'utf8');
}
...
function javaExecutable(baseDir) {
  const bundled = path.join(baseDir, 'jdk17', 'bin', 'java.exe');
...
function convertSwfsToPdf({ swfDir, pageCount, pdfPath, baseDir }) {
  ...
  const progress = { stage: 'convert', page_count: pageCount, swf_completed: [], pdf_completed: [] };
  for (let page = 1; page <= pageCount; page += 1) {
    const pageSwf = path.join(swfDir, `${page}.swf`);
    const pageOut = path.join(pdfDir, `${page}`);
    const pagePdf = path.join(pdfDir, `${page}.pdf`);
    fs.mkdirSync(pageOut, { recursive: true });
    if (existingNonEmpty(pagePdf)) {
      progress.pdf_completed.push(page);
      continue;
    }
    execFileSync(javaExecutable(baseDir), ['-jar', converters.ffdec, '-format', 'frame:pdf', '-zoom', '1', '-select', '1', '-export', 'frame', pageOut, pageSwf], { stdio: 'pipe' });
    ...
    if (!source || !existingNonEmpty(source)) throw new Error(`ffdec 未生成有效的第 ${page} 页 PDF。`);
    fs.copyFileSync(source, pagePdf);
    progress.pdf_completed.push(page);
    writeDoc88Progress(swfDir, progress);
  }
  const pagePdfs = Array.from({ length: pageCount }, (_, index) => path.join(pdfDir, `${index + 1}.pdf`));
  if (pagePdfs.some(filePath => !existingNonEmpty(filePath))) throw new Error('未找到完整的 FFDec 逐页 PDF。');
  execFileSync(converters.presse, ['merge', ...pagePdfs, '--compress', '-o', pdfPath], { stdio: 'pipe' });
...
function ensureDoc88Converters(baseDir) {
  ...
  try {
    execFileSync(javaExecutable(baseDir), ['-version'], { stdio: 'pipe' });
  } catch {
...
async function extractDoc88({ url, outputDir, baseDir }) {
  ...
  try {
    ...
    workDir = doc88WorkDir(outputDir, config.pCode);
    fs.mkdirSync(workDir, { recursive: true });
    const progress = { stage: 'download', source_url: url, p_code: config.pCode, page_count: config.pages.length, swf_completed: [], pdf_completed: [] };
    writeDoc88Progress(workDir, progress);
    for (const page of config.pages) {
      const swfPath = path.join(workDir, `${page.page}.swf`);
      if (existingNonEmpty(swfPath)) {
        progress.swf_completed.push(page.page);
        console.log(`Doc88: 复用已下载第 ${page.page}/${config.pages.length} 页`);
        continue;
      }
    ...
    return { title, pCode: config.pCode, pageCount: config.pages.length, pdfPath, method: 'resource-swf' };
  } catch (error) {
    if (workDir) error.message = `${error.message}（临时文件保留在 ${workDir}）`;
    throw error;
  } finally {
    if (workDir && fs.existsSync(pdfPath) && process.env.DOC88_KEEP_WORK !== '1') fs.rmSync(workDir, { recursive: true, force: true });
  }
}
